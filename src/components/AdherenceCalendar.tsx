...
export default function AdherenceCalendar({ doses, monthStr }: Props) {
  ...
  function navigateMonth(delta: number) {
    ...
    router.push(`/dashboard/history?month=${format(d, 'yyyy-MM')}`)
  }
  ...
  return (
    <div className="card p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => navigateMonth(-1)} className="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center text-slate-600 hover:bg-slate-200">
          ←
        </button>
